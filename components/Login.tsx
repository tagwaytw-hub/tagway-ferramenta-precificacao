...
const TagwayFullLogo = ({ className = "w-full max-w-[450px]" }: { className?: string }) => (
  ...
  >
    <g id="Camada_x0020_1">
      {/* Texto "TAGWAY TECHNOLOGY" - Branco */}
...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { alert(`Erro inesperado`); } finally { setIsLoading(false); }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input type="text" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@tagway.com.br" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-black transition-all" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Senha</label>
                <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-black transition-all" />
              </div>
            </div>
