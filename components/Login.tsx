  ...
  >
    <g id="Camada_x0020_1">
      ...
      <g id="_1949129827600">
        ...
        <path fill="#FF6600" d="M1695.98 1079.33l0 -219.87 77.19 0 0 -37.32 -191.7 0 0 37.32 77.56 0 0 219.87 36.95 0zm257.57 1.1l27.44 0 85.24 -257.92 -38.78 0c-19.02,58.17 -41.34,123.29 -61.46,184.38l-41.71 -119.26 -29.63 0 -42.07 119.63 -61.1 -184.75 -38.41 0 85.24 257.92 27.44 0c13.17,-38.78 28.9,-82.31 43.53,-124.02l44.27 124.02z" />
      </g>
    </g>
  </svg>
);
...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Login direto com o que o usuário digitar
      ...
      if (error) {
        if (error.message === 'Invalid login credentials') {
          setErrorMsg('E-mail ou Senha incorretos. Verifique se o e-mail foi confirmado no painel do Supabase.');
        } else if (error.message === 'Email not confirmed') {
          setErrorMsg('Acesso pendente: O e-mail ainda não foi confirmado.');
        } else {
    ...
    <div className="min-h-screen bg-[#000000] flex flex-col md:flex-row items-stretch overflow-hidden">
      <div className="hidden md:flex flex-[1.4] bg-[#000000] p-10 flex-col justify-center items-center relative overflow-hidden">
        ...
        <div className="relative z-10 animate-slide-up flex flex-col items-center">
          ...
          <div className="mt-12 text-slate-400 space-y-4 max-w-md text-center">
            <p className="text-xl font-medium tracking-tight leading-snug">Plataforma determinística de inteligência fiscal de alta precisão.</p>
          </div>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center p-6 md:p-8 bg-white md:rounded-l-[5rem] shadow-[-30px_0_60px_rgba(0,0,0,0.5)] z-20">
        <div className="w-full max-w-sm space-y-8 md:space-y-12">
          <div className="md:hidden text-center flex flex-col items-center"><TagwayFullLogo className="w-40" /></div>
          <div className="space-y-3">
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter">Acesso.</h2>
            <p className="text-slate-500 font-medium text-sm">Entre no sistema com seu e-mail completo.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">E-mail Completo</label>
                <input 
                  type="email" 
                  required 
                  value={email} 
            ...
            <button type="submit" disabled={isLoading} className="w-full bg-black text-white py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl transition-all btn-touch disabled:opacity-50">
              {isLoading ? 'Autenticando...' : 'Entrar'}
            </button>
          </form>
          <footer className="pt-6 md:pt-10 text-center border-t border-slate-100">
             <p className="text-[8px] md:text-[9px] font-black text-slate-300 uppercase tracking-widest leading-relaxed">© 2025 TAGWAY TECHNOLOGY — Business Intelligence<br/>Sistemas de Alta Precisão</p>
          </footer>
        </div>
      </div>
    </div>
