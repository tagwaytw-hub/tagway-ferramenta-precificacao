...
const defaultInputs: SimulationInputs = {
  ...
  ncmCodigo: '6907',
  pisCofinsRate: 9.25,
  excluirIcmsPis: false, // Desativado por padrão para bater com os R$ 212,95 da imagem do usuário
  pisCofinsVenda: 9.25,
  comissaoVenda: 0.0,
  icmsVenda: 20.50,
  outrosCustosVariaveis: 0.00,
  custosFixos: 20.00, // Ajustado de 26 para 20 conforme segunda imagem do usuário
  resultadoDesejado: 8.00,
  mode: 'tributado', // Alterado para tributado conforme o último erro relatado
  percReducaoBase: 0,
...
type Tab = 'calculadora' | 'catalogo' | 'meus-produtos' | 'overhead' | 'resumo-fiscal' | 'configuracao' | 'master' | 
           'logistica' | 'estoque' | 'metas' | 'dre' | 'caixa' | 'jarvis';
...
export const TagwayIcon = ({ size = "w-10 h-10" }: { size?: string }) => (
  <svg className={size} viewBox="0 0 1080.48 979.51" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {/* Lado Esquerdo do Cérebro - Laranja */}
  ...
  >
    ...
    {!collapsed && (
      <div className="flex-1 flex items-center justify-between overflow-hidden">
        ...
        {badge && (
          <span className="text-[7px] font-black bg-indigo-600 text-white px-1.5 py-0.5 rounded-md ml-2 shrink-0">
            {badge}
          </span>
        )}
        {isDev && !badge && (
          <span className="text-[7px] font-black bg-white/10 text-white/40 px-1.5 py-0.5 rounded-md ml-2 shrink-0">
            DEV
          </span>
        )}
      </div>
    )}
    {collapsed && (isDev || badge) && (
      <div className="absolute top-2 right-2 w-1.5 h-1.5 bg-indigo-500 rounded-full"></div>
    )}
  </button>
);

// DockItem Component for mobile navigation
const DockItem = ({ active, onClick, icon, label, isAi }: any) => (
  ...
  >
    <div className={`p-2 rounded-xl transition-all ${active ? (isAi ? 'bg-indigo-600 shadow-lg shadow-indigo-600/40' : 'bg-white/10') : ''}`}>
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d={icon} />
      </svg>
    </div>
...
const App: React.FC = () => {
  ...
  const fetchUserProfile = async (userSession: any) => {
    const { data } = await supabase.from('user_configs').select('*').eq('user_id', userSession.user.id).maybeSingle();
  ...
  const fetchOverheadConfig = async (userSession: any) => {
    ...
    if (data) {
      ...
      setIsAutoSync(!!data.is_auto_sync);
    }
  };
  ...
  // Sincronização Inteligente com a Calculadora
  useEffect(() => {
    ...
    <div className="h-screen w-full flex flex-col lg:flex-row bg-[#000000] overflow-hidden text-slate-900 relative">
      ...
      <aside className={`hidden lg:flex bg-[#000000] flex-col transition-all duration-500 z-[100] border-r border-white/5 shadow-2xl ${sidebarCollapsed ? 'w-[90px]' : 'w-[280px]'} h-screen overflow-y-auto no-scrollbar`}>
        <div className="p-6 mb-4 items-center justify-between flex border-b border-white/5">
          ...
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-2 hover:bg-white/10 rounded-lg text-white/40 transition-colors">
        ...
        <nav className="flex-1 px-4 space-y-1">
          ...
          <MenuButton active={activeTab === 'jarvis'} onClick={() => setActiveTab('jarvis')} label="Jarvis AI" collapsed={sidebarCollapsed} icon="M13 10V3L4 14h7v7l9-11h-7z" isDev badge="Q2 2026" isAi />
          ...
          <MenuButton active={activeTab === 'overhead'} onClick={() => setActiveTab('overhead')} label="Overhead" collapsed={sidebarCollapsed} icon="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2"/>
      ...
      <main className="flex-1 flex flex-col bg-[#f8fafc] lg:rounded-l-[3rem] shadow-[-20px_0_40px_rgba(0,0,0,0.1)] overflow-hidden relative">
        ...
        <header className="lg:hidden flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-md border-b border-slate-100 z-[90]">
           ...
           <button onClick={() => setIsMobileSheetOpen(true)} className="bg-black text-white p-2.5 rounded-xl shadow-lg">
        ...
        {/* View Content */}
        <div className="flex-1 overflow-y-auto flex flex-col custom-scrollbar no-scrollbar">
          {activeTab === 'calculadora' && (
            <div className="flex-1 flex flex-col lg:flex-row p-4 lg:p-12 gap-8 lg:gap-12 max-w-[1700px] mx-auto w-full animate-slide-up">
              {/* Barra de Saída / Inputs */}
              <div className="hidden lg:block w-80 space-y-6 shrink-0 h-fit sticky top-0">
          ...
          {activeTab === 'meus-produtos' && <div className="p-6 lg:p-12 space-y-4 max-w-4xl mx-auto w-full">
             {savedSimulations.map(sim => (
               <div key={sim.id} className="bg-white p-6 rounded-3xl border border-slate-200 flex justify-between items-center group shadow-sm hover:shadow-md transition-all">
                 <div>
          ...
          {/* Roadmap Views */}
          {activeTab === 'logistica' && <ComingSoonView title="Logística Inteligente" desc="Cálculo automático de fretes e integração com transportadoras." icon="M8 17a2 2 0 100 4 2 2 0 000-4zM18 17a2 2 0 100 4 2 2 0 000-4zM6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h12l4 5v3a2 2 0 01-2 2h-1" />}
          {activeTab === 'estoque' && <ComingSoonView title="Estoque Preditivo" desc="Giro de estoque e avisos de reposição automática." icon="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />}
          {activeTab === 'metas' && <ComingSoonView title="Painel de Metas" desc="Gestão de objetivos comerciais por categoria e região." icon="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />}
          {activeTab === 'dre' && <ComingSoonView title="DRE Gerencial" desc="DRE automático baseado no seu Overhead e faturamento." icon="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />}
          {activeTab === 'caixa' && <ComingSoonView title="Fluxo de Caixa" desc="Projeção de caixa real-time integrada ao sistema fiscal." icon="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />}
        </div>
        ...
        {isMobileSheetOpen && (
          <div className="lg:hidden fixed inset-0 z-[1000] flex flex-col justify-end">
             ...
             <div className="relative bg-white rounded-t-[3.5rem] p-6 max-h-[90vh] overflow-y-auto no-scrollbar shadow-2xl">
